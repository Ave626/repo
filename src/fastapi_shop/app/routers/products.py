...
import uuid
from fastapi import UploadFile, File, Form, HTTPException, status,APIRouter,Depends,Query
...
async def save_product_image(file: UploadFile) -> str:
    if file.content_type not in ALLOWED_IMAGE_TYPES:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Только JPG, PNG или WebP изображения")
    content = await file.read()
    if len(content) > MAX_IMAGE_SIZE:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "Слищком большое изображение")
...
async def get_all_products(
        ...
        updated_at : datetime | None = Query(None,description="Дата и время последнего обновления"),
...
async def update_product(
    ...
    if db_product.seller_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,detail="Вы можете менять толкьо свои товары")
    category_result = await db.scalars(select(CategoryModel).where(CategoryModel.id == product.category_id,CategoryModel.is_active == True))
    if not category_result.first():
