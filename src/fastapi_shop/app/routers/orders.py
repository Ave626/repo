...
async def checkout_order(db : AsyncSession = Depends(get_async_db),current_user : UserModel = Depends(get_current_user)):
    ...
    for cart_item in cart_items:
        ...
        if product.stock < cart_item.quantity:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Не хватает количества для товара {product.name}",
            )
        unit_price = product.price
        if unit_price is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Цена твоара {product.name} не установлена",
            )
...
async def list_orders(
    ...
    total = await db.scalars(func.count(OrderModel.id).where(OrderModel.user_id == current_user.id))
    result = await db.scalars(
        select(OrderModel)
        .options(selectinload(OrderModel.items).selectinload(OrderItemModel.product))
