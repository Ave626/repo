...
class PostService:
    ...
    async def get_post_id(self,post_id : int) -> PostRead | None:
    ...
    async def get_posts_by_category_id(self,category_id : int, skip : int = 0,limit : int = 100) -> Sequence[PostRead]:
    ...
    async def create_post(self,post_data  : PostBase) -> PostRead:
        ...
        if not category:
            raise ValueError(f"Категория с таким айди'{post_data.category_id}'не обнаружена")
