...
async def create_post_route(post_data : PostCreate,post_service : PostService = Depends(get_post_service)):
    try:
        new_post_domain = await post_service.create_post(post_data)
        return PostCreate.model_validate(new_post_domain)
    except ValueError as e:
        raise HTTPException(status_code = status.HTTP_400_BAD_REQUEST,detail = str(e))
    except Exception as e:
        raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR,detail= f"Ошибка не поддающаяся классификации: {e}")

@router.get("/{post_id}", response_model=PostRead)
async def get_post_by_id_route(post_id: int,post_service: PostService = Depends(get_post_service)):
...
async def get_all_posts_route(skip: int = 0,limit: int = 100,post_service: PostService = Depends(get_post_service)):
