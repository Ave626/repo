...
async def create_category_route(category_data : CategoryCreate,category_service : CategoryService = Depends(get_category_service)):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,detail="Ошибка не подлежащая классификации")
...
async def get_all_categories_route(skip : int = 0,limit : int = 100,category_service : CategoryService = Depends(get_category_service)):
