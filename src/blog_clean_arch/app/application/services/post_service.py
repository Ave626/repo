...
class PostService:
    ...
    async def get_post_by_id(self,post_id) -> Post | None:
    ...
    async def get_posts_by_category_id(self, category_id: int, skip: int = 0, limit: int = 100) ->List[Post]:
    ...
    async def create_post(self, post_data: PostCreate) -> Post:
        ...
        if not category:
            raise ValueError(f"Категория с таким ID {post_data.category_id} не обнаружена")
        new_post = Post(title=post_data.title,content=post_data.content,category_id=post_data.category_id)
